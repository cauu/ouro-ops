...
# E2E-12: every container runs the SAME ouro version at the fixed path.
v=$(ctl ouro version | python3 -c 'import json,sys;print(json.load(sys.stdin)["data"]["version"])')
for c in control bp1 relay1 relay2; do
  cv=$(dc exec -T "$c" ouro version | python3 -c 'import json,sys;print(json.load(sys.stdin)["data"]["version"])')
...
out1=$(ctl ouro tool run deploy/provision --dispatch bp1 --spec "$SPEC")
...
ro=$(ctl ouro tool run deploy/provision --dispatch relay1 --spec "$SPEC")
...
dc exec -T bp1 ouro audit log --limit 20 | python3 -c "
...
ctl ouro audit log --limit 20 | python3 -c "
...
out2=$(ctl ouro tool run deploy/provision --dispatch bp1 --spec "$SPEC")
...
refute_reason "tool name must be" ctl ouro tool run 'deploy/preflight; touch /tmp/pwned #' --dispatch bp1 --spec "$SPEC"
...
refute_reason "sudo" dc exec -T -u ouro-exec bp1 sudo -n /usr/local/bin/ouro confirm create --action x --machine bp1
