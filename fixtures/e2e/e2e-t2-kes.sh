...
# p2-5b: dispatch a target-bound destructive op — detect live fingerprint, issue a token bound
# to it, run with the token (the control-side gate re-detects + consumes against live evidence).
cdispatch() {
...
out1=$(cdispatch kes-rotation/rotate)
...
out2=$(cdispatch kes-rotation/rotate)
