...
- [ ] p1-1 生成 Tauri 签名密钥对（tauri signer generate），记录公钥（需用户手动执行，交互式密码输入）
- [x] p1-2 添加 tauri-plugin-updater + tauri-plugin-process 依赖（Cargo.toml + package.json）
- [x] p1-3 配置 tauri.conf.json updater（endpoints, pubkey placeholder）+ capabilities 权限（updater:default, process:allow-restart）
- [x] p1-4 注册 updater + process plugin 到 lib.rs
- [x] p1-5 新增前端更新检查逻辑（updater.ts）+ App.tsx 非阻塞更新 banner
- [x] p1-6 cargo check + pnpm build + tsc 验证通过

**Phase 2: GitHub Actions Release 工作流**
- [x] p2-1 创建 .github/workflows/release.yml（macOS ARM 构建 + Release 发布）
- [ ] p2-2 用户手动：生成签名密钥 + 配置 GitHub Secrets + 填入 pubkey
...
TC-2 | stack: node | command: pnpm -s build | result: pass | note: 415KB JS
