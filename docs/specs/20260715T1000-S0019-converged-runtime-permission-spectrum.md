...
## 29. Website-prompt Rendered SSH Policy UAT Repair (append-only)
- 2026-07-15T23:03+0800 the resumed fresh agent correctly refused a suspected lockout, but reached
  the wrong conclusion by extracting a static binary string fragment. The executable plan renders
  `AllowUsers ouro-op ouro-diag {bootstrap_user}` dynamically, so `cardano` is preserved; the
  preview did not expose that rendered non-secret policy and forced the agent outside the typed
  output contract. This is an explainability defect in the active UAT, not an actual missing-user
  defect in the installer.
- [~] p9-8 expose the exact rendered non-secret sshd access policy in onboard output, prove that
  the named bootstrap account remains allowed, and instruct the embedded onboarding Skill to use
  that typed preview field instead of inferring dynamic values from static binary strings.
- WATC-6 rendered-policy truth: an onboard preview for bootstrap user `cardano` returns the exact
  `/etc/ssh/sshd_config.d/20-ouro-s0019.conf` content and a closed `allow_users` list containing
  `ouro-op`, `ouro-diag` and `cardano`, with `bootstrap_user_preserved: true`. No secret, private
  key, confirmation value or control-key bytes are included. The embedded Skill tells a fresh
  agent to inspect this field and stop on a mismatch.
- 2026-07-15T23:07+0800 p9-8 completed: preview and real onboard output now carry a typed,
  non-secret `ssh_access_policy` generated by the same `sshd_conf(bootstrap_user)` function as the
  installed drop-in. It exposes the exact path, closed allow-user list, named bootstrap user,
  preservation boolean and rendered configuration. The embedded Skill makes this field
  authoritative and forbids static-string inference; its bundle manifest identity was regenerated.
- [x] p9-8 completed: the resumed agent can verify the actual `cardano` policy from CLI data and no
  longer needs to reverse-engineer runtime formatting from binary string fragments.

## 30. Website-prompt Rendered SSH Policy Validation Evidence (append-only)
- WATC-6 | stack: rust | command: cargo test -q -p ouro | result: pass | note: 148/148 tests; typed
  policy and the installed sshd renderer both contain `AllowUsers ouro-op ouro-diag cardano`.
- WATC-6 | stack: python | command: env PATH=/tmp/ouro-ops-test-venv/bin:$PATH make python-test |
  result: pass | note: CLI preview asserts exact drop-in path, ordered allow-user list, preserved
  bootstrap account and rendered config; control public-key bytes are absent from the policy.
- WATC-6 | stack: rust | command: cargo clippy -p ouro -- -D warnings | result: pass | note: zero
  warnings after adding the serialized policy contract.
- WATC-6 | stack: other | command: installed release `onboard --dry-run` policy assertion | result:
  pass | note: output contains `ouro-op`, `ouro-diag`, `cardano`, preservation=true and the exact
  rendered five-line config; no control-key bytes are emitted.
- WATC-6 | stack: other | command: ouro-ops manifest verify --against
  packaging/bundle-manifest.json | result: pass | note: regenerated embedded digest
  `68649a3a671e735c15a43cbecd8a30bbbd7c176041782b034d2f6218acb14a51` verifies.
- p9-8 | stack: other | command: release builds + file + shasum -a 256 + git diff --check | result:
  pass | note: installed macOS arm64 control SHA-256
