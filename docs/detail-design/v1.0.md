...
/// 探测运行中 cardano-node 容器信息（只读）
#[tauri::command]
async fn machine_runtime_probe(machine_id: i64) -> Result<RuntimeProbe, AppError>;
...
interface RuntimeProbe {
  ...
  container_name: string;        // 预期 cardano-node
  image_ref: string | null;
  ports: string[];               // 例如 "3001/tcp -> 0.0.0.0:3001"
  mounts: { destination: string; source: string }[];
  managed_by_compose: boolean;   // com.docker.compose.* labels
  bp_key_files_present: boolean; // kes/vrf/node.cert 是否存在
...
interface DeployPayload {
  ...
  takeover_existing_node?: boolean;  // 默认 false，true 时执行 takeover 预流程
  restore_snapshot?: boolean;        // 默认 false
}
```
...
### 8.2A Role: takeover（运行中节点接管）

当 `takeover_existing_node=true` 且 `safe_validation_mode=false` 时，`deploy.yml` 会在 `cardano-node` 角色前执行 `takeover` 角色，职责如下：

- 只读探测运行中 `cardano-node`（镜像、挂载、compose 标签）
- 校验接管前提挂载：
  - `/data/db`
  - `/opt/cardano/config/keys`
- 将旧宿主路径（例如 `/home/cardano/*`）幂等迁移到标准路径：
  - `/opt/cardano/db`
  - `/opt/cardano/keys`
- 写入接管日志：
  - `/opt/cardano/logs/takeover.log`
  - `/opt/cardano/logs/takeover-runtime.json`

容器切换与回滚由 `cardano-node` 角色执行：
- stop 原 `cardano-node`
- rename 为 `cardano-node-legacy-*`
- 启动新容器并健康检查
- 若失败，自动恢复 legacy 容器名称并尝试拉起（自动回滚）
...
- name: 启动 Cardano 节点容器
  community.docker.docker_container:
    ...
    env:
      ...
      CARDANO_PORT: "3001"
      RESTORE_SNAPSHOT: "{{ 'true' if (restore_snapshot | default(false) | bool) else 'false' }}"
      CARDANO_SHELLEY_KES_KEY: "{{ '/opt/cardano/config/keys/kes.skey' if role == 'bp' else omit }}"
      CARDANO_SHELLEY_OPERATIONAL_CERTIFICATE: "{{ '/opt/cardano/config/keys/node.cert' if role == 'bp' else omit }}"
      CARDANO_SHELLEY_VRF_KEY: "{{ '/opt/cardano/config/keys/vrf.skey' if role == 'bp' else omit }}"
    ports:
      - "3001:3001"
    volumes:
