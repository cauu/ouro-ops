...
interface DeployPayload {
  machine_ids: number[];
  cardano_version: string;       // Docker 镜像 tag，如 "10.2.1"
  image_registry: string;        // 镜像仓库，默认 "ghcr.io/intersectmbo/cardano-node"
...
| 文件 | 用途 | 来源 |
| :--- | :--- | :--- |
| `{network}-config.json` | 节点主配置（含 P2P 开关） | 官方 GitHub 下载，部署时 patch `EnableP2P: true` |
| `{network}-byron-genesis.json` | Byron 创世配置 | 官方 GitHub 下载，不修改 |
| `{network}-shelley-genesis.json` | Shelley 创世配置 | 官方 GitHub 下载，不修改 |
| `{network}-alonzo-genesis.json` | Alonzo 创世配置 | 官方 GitHub 下载，不修改 |
| `{network}-conway-genesis.json` | Conway 创世配置 | 官方 GitHub 下载，不修改 |
| `topology.json` | P2P 拓扑 | 本地 Jinja2 模板渲染（见 §8.5） |

**下载 URL 模式：**
```
https://book.play.dev.cardano.org/environments/{{ network }}/{{ filename }}
```
...
- name: 下载官方网络配置文件
  get_url:
    url: "https://book.play.dev.cardano.org/environments/{{ network }}/{{ item }}"
    dest: "/opt/cardano/config/{{ item }}"
    mode: '0644'
  loop:
    - "{{ network }}-config.json"
    - "{{ network }}-byron-genesis.json"
    - "{{ network }}-shelley-genesis.json"
    - "{{ network }}-alonzo-genesis.json"
    - "{{ network }}-conway-genesis.json"

- name: 启用 P2P 模式 (patch config.json)
  # 官方默认配置可能未开启 P2P，强制 patch
  shell: |
    jq '.EnableP2P = true' \
      /opt/cardano/config/{{ network }}-config.json > /tmp/config-patched.json
    mv /tmp/config-patched.json /opt/cardano/config/{{ network }}-config.json
  args:
    executable: /bin/bash
...
- name: 生成 P2P 拓扑配置
  ...
  # P2P 模式：Relay 启用 useLedgerAfterSlot 自动发现 peers；
  # BP 仅配置 localRoots 指向自己的 Relay，不启用 ledger peers。
  # 模板根据 role 变量自动切换配置。
...
> **注意：** P2P 模式要求 `cardano-node` 主配置文件中 `EnableP2P: true`。部署 Role 会同时渲染 `{network}-config.json`，确保该字段已开启。`TraceOptionPeerFrequency` 等 P2P 相关 trace 参数也会一并配置，便于后续监控模块采集 peer 连接质量数据。
...
- name: 回滚 Cardano 节点
  ...
  tasks:
    ...
    - name: 用旧版本镜像启动容器
      community.docker.docker_container:
        name: cardano-node
        image: "ghcr.io/intersectmbo/cardano-node:{{ previous_version }}"
        state: started
        restart_policy: unless-stopped
        ports:
          - "3001:3001"
